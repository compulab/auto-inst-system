...
sed '1,/^\[ fw_env.config/d;/^\[/,$d' /media/source/config > /etc/fw_env.config
# Extract the installation media boot command
first_boot=$( awk '/^first_boot/ { gsub(/.*=/, "", $1); print }' /media/source/config )
# Extract the environment unlock device
unlock_dev=$( awk '/^unlock_dev/ { gsub(/.*=/, "", $1); print }' /media/source/config )
